...
# ================= 🚀 强制环境配置 (新增) =================
# 1. 彻底禁用 NCCL P2P (解决 Illegal Memory Access 核心)
os.environ["NCCL_P2P_DISABLE"] = "1"
# 2. 禁用 InfiniBand (防止容器网络报错)
os.environ["NCCL_IB_DISABLE"] = "1"
# 3. 禁用 Tokenizer 并行 (防止死锁)
...
def main():
    ...
    if len(cur_ckpts) > 0 and not configs.only_eval:
        if rank == 0:
            print(f"Warning: found previous run and gonna resume from that.")
    ...
    # --- 3. 【关键调整】优先加载 Tokenizer 和 处理数据 (CPU任务) ---
    # 先做这一步，确保此时 GPU 是空闲的，防止 FSDP 冲突
    if rank == 0:
        print("Initializing tokenizer and processing dataset BEFORE model loading...")
    ...
    for d in val_data:
        ...
        elif "input" in d: q = d["input"]
        else: raise KeyError(f"No question/input field")
        if "answer" in d: a = d["answer"]
        elif "output" in d: a = d["output"]
        else: raise KeyError(f"No answer/output field")
        if "steps" in d and isinstance(d["steps"], list): c = "\n".join(d["steps"])
        elif "cot" in d: c = d["cot"]
        else: c = ""
        question_val.append(q)
        answers_val.append(str(a).replace(",", "").strip())
        cot_val.append(c)

    # 加载数据集对象 (此时还在 CPU 上跑 map)
    ...
    # --- 4. 加载模型 (GPU任务) ---
    if rank == 0:
        print("Dataset processed. Now loading model to GPU...")
