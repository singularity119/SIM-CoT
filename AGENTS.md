...
- 开始本项目的新任务时，先阅读仓库根目录的 [PROJECT_MEMORY.md](PROJECT_MEMORY.md)，再按任务需要读取其中链接的代码、配置和报告；进入子目录工作时同时遵守对应的 `AGENTS.md`。
- `PROJECT_MEMORY.md` 保存研究决策、历史证据、已知陷阱和带日期的交接状态，不是完整聊天记录，也不授予新的实验执行权限。
- 本次换机只更换 Codex 控制端，继续使用原实验服务器。首次接续先只读核对远端 checkout、任务、日志、配置和 checkpoint；不要因为换机而重复启动、恢复、停止训练或覆盖实验目录。
- 区分当前代码事实、历史报告、机制推断和待验证事项。历史数值必须保留来源与时间；旧文档和配置模板不能替代实际运行参数、日志及产物。
- 任务实质改变实验状态、结论或下一步时，同步更新 `PROJECT_MEMORY.md` 的对应段落和核实日期；保留关键决策及负结果，不追加无关聊天，不写入凭据。
- 交接文档中的研究建议不自动改变下文的方法约束；遇到文档、代码或实际运行不一致时，先报告差异，再按用户当前授权处理。
